...
export default function BranchVisualizer({ documentId, provider }: BranchVisualizerProps) {
  ...
  const handleBranch = async () => {
    ...
    setError("");
    try {
      // 1. Create the branch doc record in the backend
      const newDoc = await fetchApi(`/docs/${documentId}/branch`, { method: "POST" });

      // 2. Capture current CRDT state
      const stateUpdate = Y.encodeStateAsUpdate(provider.document);

      // 3. Get the WS URL from the provider's configuration
      const wsUrl = (provider.configuration as any)?.url || 
                    process.env.NEXT_PUBLIC_WS_URL || 
                    "ws://localhost:1234";

      // 4. Create a temporary provider to push the state to the new doc room
      ...
      const tempProvider = new HocuspocusProvider({
        ...
        document: tempDoc,
        onSynced: () => {
          // Give a moment for the server to persist the state
          setTimeout(() => {
            tempProvider.destroy();
            tempDoc.destroy();
            setSuccess(true);
            setTimeout(() => router.push(`/doc/${newDoc.id}`), 500);
          }, 800);
        },
      });

      // Fallback: navigate even if WS doesn't connect (offline mode)
      setTimeout(() => {
        if (!success) {
          try { tempProvider.destroy(); tempDoc.destroy(); } catch {}
  ...
  if (error) {
    return (
      <div className="text-xs text-rose-400 px-2 py-1 rounded-lg bg-rose-500/10 border border-rose-500/20">
        {error}
      </div>
    );
  }
  ...
  return (
    <button
      ...
      disabled={branching}
      title="Create a branch of this document (Ctrl+D)"
      ...
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLButtonElement).style.background = success
          ? "rgba(16,185,129,0.1)"
          : "rgba(99,102,241,0.1)";
      }}
    >
